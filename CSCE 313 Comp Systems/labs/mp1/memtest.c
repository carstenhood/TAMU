...
#include<stdlib.h>
#include<stdio.h>
#include<unistd.h>  // for getopt C library function
#include<string.h>
#include<sys/time.h>
#include<assert.h>
...
// need void return value to utilize atexit() on release_allocator()
void release_allocator_wrapper() { release_allocator(); }
...
int main(int argc, char ** argv) {
    atexit(release_allocator_wrapper);
    
    // initialize sizes to default values;
    unsigned int block_size = 128;      // 128 bytes
    unsigned int pool_size = 128*1024;  // 512 kb1
    ...
    while ((c = getopt(argc, argv, "b:s:")) != -1) {
        switch (c) {
            case 'b':
                ...
                break;
            case 's':
                pool_size = (int)atoi(optarg);
                break;
        }
    }
    printf("using block size: %d, pool size: %d\n", block_size, pool_size);
    
    // run test
    init_allocator(block_size, pool_size);
    ackerman_main();
    // release function is automatically called via atexit()
