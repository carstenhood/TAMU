...
//Conner Fear, Chance Gray,
//Matt Hegarty, Carsten Hood
...
// search and edit initializations
...
rows_text(Point(display_x+display_width+40, h-130), "Display Sizing"),
...
{
    //attach search and edit objects
...
{
    //initialize vector and label of available movie labels/tags
    ...
    Text *all_tags_text = new Text(Point(20, 785), tags_stream.str());
...
//functions for hiding and showing buttons when switching between edit and search
...
void Movie_window::search_by_actor()
{
    clear_search();
    try {
        Actor *actor = (Search_Movies_By_Actor(search_actor.get_string(), movie_vec));
...
//other actions invoked by callbacks
...
//sets sizing of movies and determines how many fit into a page
...
//display movie and actor information
...
void Movie_window::display_movies(int index)
{
    ...
    double y = 0;
     ...
     //loop through every position in the display grid and call draw_movie
...
// display actions invoked by callback functions
...
int main() {
    try {
        ...
        Movie_window win(Point(0,0), 1300, 800, "Movie Database");
        return gui_main();
    } catch(exception& e) {
