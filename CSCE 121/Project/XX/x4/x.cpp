...
//testing code begins
//-----------------------------------------------------------------
Simple_window win1(Point(100,200), 1400, 800, "Testing");
...
Image *actor_photo;
Text *actor_name;
int rows=3; //global variable can be 1-4; its square is the # of movies on the screen at a time;
int display_index;
double display_x = 550; //leftmost point of display (probs around 300)
double display_y = 10; //margin of top of display (probs around 10)
double display_width = 600;
double display_height = 770;
...
void clear_actor()
{
    win1.detach(*actor_name);
    win1.detach(*actor_photo);
    delete actor_name;
    delete actor_photo;
}
...
void display_actor(Actor a)
{
    ...
    win1.attach(*actor_name);
    win1.attach(*actor_photo);
}
...
int main()
{
    try {
        populate_vectors();
        display_movies(0);
        display_actor(movie_vector[0].actors[0]);
