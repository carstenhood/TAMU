...
//--------------------------------------------------------------------------------------------------------------------
vector<int> Search_Movies (string Actors, string Title, string Yearstring, string Tags, const vector<Movie>& movie_vec) {
try
{
		...
		for (int a=0; a<actor_names.size(); ++a)			//Checking actors
		{
			for (int b=0; b<movie_vec.size(); ++b)			//Checks movies
			{
				for (int z=0; z<movie_vec[b].actors.size(); ++z){			//Checks EACH actor
					for (int q=0; q<movie_vec[b].actors[z].names.size(); ++q)			//Checks EACH actor
					{
		...
		while (true)
	{
			if(Tags == "")
				...
				for (int b=0; b<movie_vec.size(); ++b)			//Adds matching movies to the vector of ints
				{
					...
					{
						for (int q=0; q<movie_vec[b].tags[z].size(); ++q)
						{
						if (movie_vec[b].tags[z] == tags_vec[q]) {				//Needs to be a double depth check to check all actors.
							int_vector.push_back(b);
							break;			
							}
						}
...
Movie_window::Movie_window(Point xy, int w, int h, const string& title)
	...
	update_button(Point(300,350), 70, 20, "Update", cb_update),
...
void Movie_window::create(){
	...
	for(int i=0;i<m.actors.size();++i){
		for(int j=0;j<m.actors[i].names.size();++j){
			cout<<m.actors[i].names[j];
...
try {
    ...
    Movie_window win(Point(0,0),1300,1000,"lines");
    return gui_main();
}
catch(exception& e) {
